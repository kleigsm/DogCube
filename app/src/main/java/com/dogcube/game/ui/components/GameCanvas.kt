...
fun GameCanvas(
    ...
    Canvas(modifier = modifier.fillMaxWidth().aspectRatio(Board.WIDTH.toFloat() / Board.HEIGHT.toFloat()).padding(horizontal = 16.dp)) {
        ...
        val bc = p.breed.color
        val sc = bc.copy(red = bc.red * 0.6f, green = bc.green * 0.6f, blue = bc.blue * 0.6f)
        ...
        for (row in shape.indices) for (col in shape[row].indices) {
            ...
            drawRect(bc, Offset(ox + 1, oy + 1), Size(cw - 2, ch - 2))
            drawLine(bc.copy(alpha = 0.5f), Offset(ox + 1, oy + 1), Offset(ox + cw - 1, oy + 1), 1.5f)
            drawLine(bc.copy(alpha = 0.5f), Offset(ox + 1, oy + 1), Offset(ox + 1, oy + ch - 1), 1.5f)
            drawLine(sc.copy(alpha = 0.3f), Offset(ox + 1, oy + ch - 1), Offset(ox + cw - 1, oy + ch - 1), 1.5f)
            drawLine(sc.copy(alpha = 0.3f), Offset(ox + cw - 1, oy + 1), Offset(ox + cw - 1, oy + ch - 1), 1.5f)
        }
    }
}
